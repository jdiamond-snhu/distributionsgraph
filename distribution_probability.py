...
# Let the user choose the Risk-Free Rate behavior interactively
rf_selection = st.sidebar.radio(
    "Select Risk-Free Hurdle Rate:",
    ["Live Market Rate (Dynamic)", "Static 3.0% (Stable Baseline)"]
)

# Fetch active risk-free rate based on selection
if rf_selection == "Live Market Rate (Dynamic)":
    rf_rate = get_risk_free_rate()
else:
    rf_rate = 0.030

st.sidebar.markdown(f"**Active Risk-Free Rate Used:** {rf_rate:.2%}")
st.sidebar.markdown("---")
